...
#Solver is saga, C is 0.3, Accuracy is 0.9994413407821229, Precision is 0.9977272727272727, Recall is 0.99975, F1_score is 0.9986838956688203, Accs is [0.9949779271799442, 0.9723106742350823, 0.9555555555555556, 1.0]
#Solver is newton-cg, C is 0.1, Accuracy is 0.9972067039106145, Precision is 0.994484698469847, Recall is 0.9920804473304473, F1_score is 0.9931044956763018, Accs is [0.9955854359888219, 0.9844939775716461, 0.7555555555555555, 0.9629629629629629]
#Solver is sag, C is 0.1, Accuracy is 0.997209807572936, Precision is 0.9965940734698469, Recall is 0.9889058441558441, F1_score is 0.9923687788858399, Accs is [0.9955854359888219, 0.9847708708292953, 0.8444444444444444, 0.9629629629629629]
def main():
    n = 5000
    #ifile = './GroundtruthData/bgp_classifier.'+str(n)+'.2f.csv'
    ifile = './GroundtruthData/FourClasses/new_features/bgp_classifier.5000.2f.csv'
    ...
    # 1655878642,95.214.110.0/24,201153,0,1,0,0.3,0.0,0.16259743447766142,0.0
    #dt_grid_searching(X, y, n)
    # there is no need to perform cross validation on mini dataset!!!!!!!!!
    #svm_grid_searching(X, y, n)
    #mlr_grid_searching(X, y, n)

    #multiLR_classifier(X, y, s='lbfgs', c=0.9)
    dt_classifier(X, y, d=3, l=2)

    #svm_classifier(X, y, c=600)

    clf = 'dt'
    #accs = test_validate_data(clf)
    # print(accs)
    accs = test_bgp_incidents(clf)
    print(accs)
