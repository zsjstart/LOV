...
def load_caida_as_rel(ifile):
    caida_as_rel_pc = defaultdict(set)
    caida_as_rel_cp = defaultdict(set)
    caida_as_rel_pp = defaultdict(set)
    IXP_ASes = []
...
def download_as_rel_v6(y, m, d):
    download_path = "https://publicdata.caida.org/datasets/as-relationships/serial-1/" + \
        y+m+d+".as-rel.v6-stable.txt.bz2"
    save_path = "./LocalData/CAIDA/as-rel.v6-stable.txt.bz2"
    output = "./LocalData/CAIDA/as-rel.v6-stable.txt"
    response = requests.get(download_path)
    if response.status_code == 200:
        with open(save_path, "wb") as f:
            f.write(response.content)
    ...
    caida_as_rel_pp_0 = {}
    with open('./LocalData/CAIDA/caida_as_rel_pp.p', "rb") as f:
        caida_as_rel_pp_0 = pickle.load(f)
    caida_as_rel_pc_0 = {}
    with open('./LocalData/CAIDA/caida_as_rel_pc.p', "rb") as f:
        caida_as_rel_pc_0 = pickle.load(f)
    for key, values in caida_as_rel_pp_0.items():
    	caida_as_rel_pp[key].update(values)
    for key, values in caida_as_rel_pc_0.items():
    	caida_as_rel_pc[key].update(values)
...
def download_prefixes_to_as(y, m, d):
    ...
    download_path = "https://publicdata.caida.org/datasets/routing/routeviews-prefix2as/" + \
        y+"/"+m+"/"+"routeviews-rv2-"+y+m+d+"-1200.pfx2as.gz"
    print(download_path)
...
def process_routing_table(infile):
    dic = dict()
...
def download_bgp_routing_table(y, m, d):
    download_path = "http://archive.routeviews.org/route-views.amsix/bgpdata/" + \
        y+"."+m+"/"+"RIBS/rib."+y+m+d+".1800.bz2"
    print(download_path)
    save_path = "./bgpdata/rib."+y+m+d+".1800.bz2"
    '''
    response = requests.get(download_path)
    if response.status_code == 200:
        with open(save_path, "wb") as f:
            f.write(response.content)
    '''
    outfile = save_path.replace('.bz2', '.txt')
    cmd = """ bgpscanner %s > %s """ % (save_path, outfile)
    os.system(cmd)
    dic = process_routing_table(outfile)
    print(len(dic))
...
def load_ihr_local_v2(ifile):
    local_hege_dict = {}
    df = pd.read_csv(ifile)
    for timebin, originasn, asn, hege in zip(df.iloc[:, 0].values, df.iloc[:, 1].values, df.iloc[:, 2].values, df.iloc[:, 3].values):
        local_hege_dict[(originasn, asn)] = hege
    os.remove(ifile)
    return local_hege_dict
...
def update_at_specific_times(y, m, d):
    ...
    #    global_hege_dict = pickle.load(f)
    return roa_px_dict, as2prefixes_dict
...
def main():
    ...
    #download_bgp_routing_table('2022', '10', '02')
